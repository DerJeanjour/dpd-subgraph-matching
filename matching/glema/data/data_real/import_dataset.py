...
def import_datasets( args ):
    ...
    for dataset_filename in list_datasets:
        ...
        graph_idx = read_dataset_to( args, dataset_name, G, node_id_mapping, pivot_mapping, graph_idx )

    print( f"Collected all datasets: Nodes[{len( G.nodes )}] / Edges[{len( G.edges )}]" )
    if len( G.nodes ) == 0:
        return

    write_files( args, G, node_id_mapping, pivot_mapping )
...
def read_dataset_to( args,
    ...
    for pivot_node_id, node_data in tqdm( list( G.nodes( data=True ) ) ):
        ...
        G_sub = get_k_neighbourhood( G, pivot_node_id, args.import_subgraph_radius,
        ...
        add_graph_to( G_sub, G_target,
                      node_id_mapping, pivot_mapping,
                      pivot_node_id, graph_idx )  # subgraph around record
        graph_idx += 1

    return graph_idx


def get_k_neighbourhood( G: nx.DiGraph, pivot_node_id, k: int, min_n: int = 2, max_n: int = 50 ):
    resize_tries = 0
    max_resize_tries = 10
    subgraph_radius = k
    G_sub = nx.ego_graph( G, pivot_node_id, radius=subgraph_radius, undirected=True )
...
def add_graph_to( G_source: nx.DiGraph, G_target: nx.DiGraph,
                  node_id_mapping: dict[ str, int ], pivot_mapping: dict[ int, int ],
                  pivot_node_id: str, graph_idx: int ):
    # process nodes
    for node_idx, node in enumerate( G_source.nodes( data=True ) ):
        ...
        if node_id == pivot_node_id:
            pivot_mapping[ graph_idx ] = node_id_mapping[ node_id_key ]
...
def write_files( args, G: nx.DiGraph, node_id_mapping: dict[ str, int ], pivot_mapping: dict[ int, int ], ):
    ...
    pivot_id_file = os.path.join( output_dir, f"{args.dataset}.pivots" )
    with open( pivot_id_file, "w", encoding="utf-8" ) as file:
        for graph_idx, pivot_node_id in pivot_mapping.items():
            file.write( f"{graph_idx} {pivot_node_id}\n" )
