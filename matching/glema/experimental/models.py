...
class OrderEmbedder( nn.Module ):
    ...
    def criterion( self, pred, intersect_embs, labels ):
        ...
        emb_as, emb_bs = pred
        e = torch.sum( torch.max( torch.zeros_like( emb_as,
                                                    device=model_utils.get_device() ), emb_bs - emb_as ) ** 2, dim=1 )

        margin = self.margin
        e[ labels == 0 ] = torch.max( torch.tensor( 0.0,
                                                    device=model_utils.get_device() ), margin - e )[ labels == 0 ]
...
class InferenceGNN:
    ...
    def to_input( self, batch ):
        ...
        sources = process_batch( sources )
        queries = process_batch( queries )

        sources, queries = (sources.to( self.device ), queries.to( self.device ))
    ...
    def prepare_multi_input( self, sources, queries ):
        ...
        for source, query in zip( sources, queries ):
            inputs.append( self.prepare_single_input( source, query ) )

        return inputs

    def predict_label( self, sources, queries ):
        inputs = self.prepare_multi_input( sources, queries )
        inputs = self.to_input( inputs )
        ...
        with torch.no_grad():
            ...
            pred = self.model( emb_sources, emb_queries )
            raw_pred = self.model.predict( pred )
            pred = self.model.clf_model( raw_pred.unsqueeze( 1 ) ).argmax( dim=-1 )
